...
Шаблон содержит один эталонный 2-рядный блок (ряды 6-7), который генератор клонирует под каждый резервуар. Подпись (E9:J9 + M9:Q9) автоматически уезжает вниз.
...
| R | харорат | temperature °C (current hour) | `reservoir_flood_hourly.temperature_c` |
| S | Диспетчер Ф.И.Ш | duty (current, fallback prev) | `reservoir_flood_hourly.duty_name` |

Ряд 7 (и клонированные `9, 11, …, 23`) — формулы дельты `=IFERROR(D6-C6, "-")` по парам колонок. Excelize при `DuplicateRowTo` копирует формулу буквально (без сдвига ссылок), поэтому генератор после клонирования **перезаписывает** дельту корректным `=IFERROR(D{value_row}-C{value_row}, "-")` для каждого скопированного блока.
...
- `S3` хранит дату.
- Подзаголовки времени C5..O5 — формулы `=MOD($S$2-1/24, 1)` (час−1, корректный wrap через полночь).
- Подзаголовки D5..P5 — `=$S$2`.

Это значит: запрос `?hour=15` приведёт к `S2 = 15:00`, а формулы C5..O5 покажут `14:00`, D5..P5 — `15:00`. При `?hour=0` → `S2 = 00:00`, формулы дают `23:00 / 00:00`.
...
Внизу отчёта генератор пишет `ShortenName(claims.Name)` (`Иванов Иван Иванович` → `И. Иванов`) в ячейку `M{signer_row}`. `signer_row` сдвигается вниз вместе с количеством блоков: для 9 резервуаров — ряд 25.
