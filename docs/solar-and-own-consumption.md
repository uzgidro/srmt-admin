...
| 401 | Нет/протух токен | `{"error": "unauthorized"}` |
| 403 | Роль не sc/rais | `{"error": "forbidden"}` |
| 404 | `organization_id` не существует | `{"error": "organization not found"}` |
| 500 | Ошибка БД | `{"error": "internal server error"}` |
...
cascade видит планы только своей org.
...
В `aggregations` ответа появились два новых поля:
