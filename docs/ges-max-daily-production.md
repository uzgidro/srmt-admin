...
Backend проверяет cap **только для того значения, которое юзер реально прислал**:

| Что в payload'е | Что бэк сравнивает с cap |
| --- | --- |
| `daily_production_mln_kwh: 10.0` | 10.0 |
| Поле опущено (partial update) | **не проверяется** — historical БД-значение не трогается |
| Поле явно `null` | 0 (всегда проходит) |

Это значит: **partial-update без `daily_production_mln_kwh` гарантированно проходит cap-валидацию**, даже если в БД уже лежит значение, превышающее текущий cap. Это сознательное поведение: юзер не должен быть наказан историческими данными за то, что он пытается обновить совсем другое поле (например, `own_consumption_kwh`).

> **Важное следствие.** Если админ опустил cap, старые значения в БД, превышающие новый cap, не будут принудительно «вылечены» при следующем partial-update. Они останутся в строке. Чтобы их исправить — нужен явный PATCH с `daily_production_mln_kwh: <новое значение>` (≤ cap). Это разовая операция, делается админом или скриптом, не побочный эффект случайного PATCH.
