...
> сквозной доступ ко всем организациям; для будущих ролей действует
> ограничение по `claims.OrganizationIDs`.
...
| `from` | `YYYY-MM-DD` | start_time ≥ начало этой даты (UTC) |
| `to` | `YYYY-MM-DD` | start_time ≤ полночь этой даты (UTC) — **граница исключающая**: события на саму дату `to` НЕ попадают в выборку |

> Бэк парсит дату как `time.Parse("2006-01-02", s)` → midnight UTC. Чтобы
> включить **весь** июнь 2026 — `from=2026-06-01&to=2026-07-01` (день+1).
> Чтобы получить ровно одну дату — `from=2026-06-08&to=2026-06-09`.
