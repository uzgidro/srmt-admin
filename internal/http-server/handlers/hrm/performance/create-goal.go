...
func CreateGoal(log *slog.Logger, svc GoalCreator) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		const op = "handlers.hrm.performance.CreateGoal"
		...
		id, err := svc.CreateGoal(r.Context(), req)
		if err != nil {
			log.Error("failed to create goal", sl.Err(err))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, resp.InternalServerError("Failed to create goal"))
			return
		}
