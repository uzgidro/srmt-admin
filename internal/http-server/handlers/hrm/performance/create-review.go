...
func CreateReview(log *slog.Logger, svc ReviewCreator) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		const op = "handlers.hrm.performance.CreateReview"
		...
		id, err := svc.CreateReview(r.Context(), req)
		if err != nil {
			log.Error("failed to create review", sl.Err(err))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, resp.InternalServerError("Failed to create review"))
			return
		}
