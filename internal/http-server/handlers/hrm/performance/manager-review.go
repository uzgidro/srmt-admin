...
type ManagerReviewer interface {
	ManagerReview(ctx context.Context, id int64, req dto.ManagerReviewRequest) error
}

func ManagerReview(log *slog.Logger, svc ManagerReviewer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		const op = "handlers.hrm.performance.ManagerReview"
		...
		var req dto.ManagerReviewRequest
		...
		if err := svc.ManagerReview(r.Context(), id, req); err != nil {
			...
			if errors.Is(err, storage.ErrInvalidStatus) {
				...
				render.JSON(w, r, resp.Conflict("Review is not in manager_review status"))
				return
			}
