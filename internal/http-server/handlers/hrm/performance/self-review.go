...
type SelfReviewer interface {
	SelfReview(ctx context.Context, id int64, req dto.SelfReviewRequest) error
}

func SelfReview(log *slog.Logger, svc SelfReviewer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		const op = "handlers.hrm.performance.SelfReview"
		...
		if err := svc.SelfReview(r.Context(), id, req); err != nil {
			...
			if errors.Is(err, storage.ErrInvalidStatus) {
				...
				render.JSON(w, r, resp.Conflict("Review is not in self_review status"))
				return
			}
