...
type VacationUpdater interface {
	Update(ctx context.Context, id int64, req dto.EditVacationRequest) error
}

func Update(log *slog.Logger, svc VacationUpdater) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		const op = "handlers.hrm.vacation.Update"
		...
		var req dto.EditVacationRequest
		...
		if err := svc.Update(r.Context(), id, req); err != nil {
			...
			if errors.Is(err, storage.ErrInvalidStatus) {
				render.Status(r, http.StatusBadRequest)
				render.JSON(w, r, resp.BadRequest("Vacation can only be updated in draft or pending status"))
				return
			}
