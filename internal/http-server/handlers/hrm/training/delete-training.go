...
func DeleteTraining(log *slog.Logger, svc TrainingDeleter) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := svc.DeleteTraining(r.Context(), id); err != nil {
			...
			if errors.Is(err, storage.ErrInvalidStatus) {
				render.Status(r, http.StatusBadRequest)
				render.JSON(w, r, resp.BadRequest("Cannot delete training that is in progress or completed"))
				return
			}
