...
type AssessmentCompleter interface {
	CompleteAssessment(ctx context.Context, id int64) error
}

func CompleteAssessment(log *slog.Logger, svc AssessmentCompleter) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := svc.CompleteAssessment(r.Context(), id); err != nil {
			...
			if errors.Is(err, storage.ErrInvalidStatus) {
				render.Status(r, http.StatusBadRequest)
				render.JSON(w, r, resp.BadRequest("Assessment can only be completed from in_progress status"))
				return
			}
