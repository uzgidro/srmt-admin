...
type ZoneCreator interface {
	CreateZone(ctx context.Context, req dto.CreateAccessZoneRequest) (int64, error)
}

func CreateZone(log *slog.Logger, svc ZoneCreator) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		const op = "handlers.hrm.access.CreateZone"
		log := log.With(slog.String("op", op), slog.String("request_id", middleware.GetReqID(r.Context())))

		var req dto.CreateAccessZoneRequest
