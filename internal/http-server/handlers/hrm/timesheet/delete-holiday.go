...
type HolidayDeleter interface {
	DeleteHoliday(ctx context.Context, id int64) error
}

func DeleteHoliday(log *slog.Logger, svc HolidayDeleter) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := svc.DeleteHoliday(r.Context(), id); err != nil {
			if errors.Is(err, storage.ErrHolidayNotFound) {
