...
type TurnoverGetter interface {
	GetTurnoverReport(ctx context.Context, filter dto.ReportFilter) (*analytics.TurnoverReport, error)
}

func GetTurnover(log *slog.Logger, svc TurnoverGetter) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		const op = "handlers.hrm.analytics.GetTurnover"
		log := log.With(slog.String("op", op), slog.String("request_id", middleware.GetReqID(r.Context())))

		filter := parseReportFilter(r)

		result, err := svc.GetTurnoverReport(r.Context(), filter)
		if err != nil {
