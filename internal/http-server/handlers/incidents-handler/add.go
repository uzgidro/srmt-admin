...
type incidentAdder interface {
	AddIncident(ctx context.Context, req dto.AddIncidentRequest, files []*multipart.FileHeader) (int64, []fileupload.UploadedFileInfo, error)
}

func Add(log *slog.Logger, adder incidentAdder, _ fileupload.FileUploader, _ fileupload.FileMetaSaver, _ fileupload.CategoryGetter) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var req dto.AddIncidentRequest
		...
		if formparser.IsMultipartForm(r) {
			...
			req.IncidentTime, err = formparser.GetFormDateTimeRequired(r, "incident_time")
			if err != nil {
				render.Status(r, http.StatusBadRequest)
			...
			if err != nil {
				render.Status(r, http.StatusBadRequest)
				render.JSON(w, r, resp.BadRequest("Invalid description"))
				return
			}
		...
		// Call Service
		id, uploadedFiles, err := adder.AddIncident(r.Context(), req, files)
		if err != nil {
