...
func Add(log *slog.Logger, adder reportAdder, uploader fileupload.FileUploader, saver fileupload.FileMetaSaver, categoryGetter fileupload.CategoryGetter) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			if uploadResult != nil {
				log.Warn("report creation failed, compensating uploaded files")
				fileupload.CompensateEntityUpload(r.Context(), log, uploader, saver, uploadResult)
			}
		...
		if len(fileIDs) > 0 {
			if err := adder.LinkReportFiles(r.Context(), id, fileIDs); err != nil {
...
func parseMultipartAddRequest(
	...
	const op = "reports.parseMultipartAddRequest"
