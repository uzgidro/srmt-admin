...
func Add(log *slog.Logger, adder letterAdder, uploader fileupload.FileUploader, saver fileupload.FileMetaSaver, categoryGetter fileupload.CategoryGetter) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		const op = "handlers.letters.add"
		...
		storageReq := dto.AddLetterRequest{
			Name:                 req.Name,
			Number:               req.Number,
		...
		id, err := adder.AddLetter(r.Context(), storageReq, userID)
		if err != nil {
			if uploadResult != nil {
				log.Warn("letter creation failed, compensating uploaded files")
				fileupload.CompensateEntityUpload(r.Context(), log, uploader, saver, uploadResult)
			}
			...
			log.Error("failed to add letter", sl.Err(err))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, resp.InternalServerError("Failed to add letter"))
			return
		}

		if len(fileIDs) > 0 {
			if err := adder.LinkLetterFiles(r.Context(), id, fileIDs); err != nil {
				log.Error("failed to link files", sl.Err(err))
			}
		}

		if len(req.LinkedDocuments) > 0 {
			if err := adder.LinkLetterDocuments(r.Context(), id, req.LinkedDocuments, userID); err != nil {
		...
		log.Info("letter added successfully",
			slog.Int64("id", id),
...
func parseMultipartAddRequest(
	...
	const op = "letters.parseMultipartAddRequest"
