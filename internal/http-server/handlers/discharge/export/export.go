...
import (
	...
	excelgen "srmt-admin/internal/lib/service/excel/discharge"
...
// New returns an HTTP handler for Excel/PDF export of discharge reports
func New(log *slog.Logger, getter DischargeGetter, generator *excelgen.Generator, loc *time.Location) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		const op = "handlers.discharge.export.New"
		log := log.With(
			slog.String("op", op),
		...
		// Validate date format and calculate period (7:00 to 7:00 next day in UTC+5)
		...
		// Operational day starts at 05:00 local time and ends at 05:00 next day
		startDate := time.Date(parsedDate.Year(), parsedDate.Month(), parsedDate.Day(), 5, 0, 0, 0, loc)
		endDate := startDate.Add(24 * time.Hour)

		// Fetch discharge data for the operational day
		data, err := getter.GetAllDischarges(r.Context(), nil, &startDate, &endDate)
		if err != nil {
		...
		// Generate Excel file
		excelFile, err := generator.GenerateExcel(dateStr, data, loc)
		if err != nil {
		...
		if format == "excel" {
			...
			filename := fmt.Sprintf("Холостые-сбросы-%s.xlsx", parsedDate.Format("2006-01-02"))
			...
			log.Info("successfully generated Excel export",
				...
				slog.Int("discharge_count", len(data)),
			)
		} else {
			// PDF export
...
func exportPDF(w http.ResponseWriter, excelFile *excelize.File, parsedDate time.Time, log *slog.Logger) error {
	...
	tempDir, err := os.MkdirTemp("", "discharge-pdf-*")
	if err != nil {
	...
	excelPath := filepath.Join(tempDir, "discharge.xlsx")
	if err := excelFile.SaveAs(excelPath); err != nil {
		return fmt.Errorf("failed to save Excel file: %w", err)
	}

	// Convert Excel to PDF using LibreOffice
	pdfPath := filepath.Join(tempDir, "discharge.pdf")
	...
	filename := fmt.Sprintf("Холостые-сбросы-%s.pdf", parsedDate.Format("2006-01-02"))
