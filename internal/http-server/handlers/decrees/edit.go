...
func Edit(log *slog.Logger, editor decreeEditor, uploader fileupload.FileUploader, saver fileupload.FileMetaSaver, categoryGetter fileupload.CategoryGetter) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			if uploadResult != nil {
				log.Warn("decree update failed, compensating uploaded files")
		...
		if hasFileChanges {
			...
			if len(fileIDs) > 0 {
				if err := editor.LinkDecreeFiles(r.Context(), id, fileIDs); err != nil {
...
func parseMultipartEditRequest(
	...
	const op = "decrees.parseMultipartEditRequest"
