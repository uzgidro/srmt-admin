...
func Update(log *slog.Logger, updater LocationUpdater) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var req filtration.UpdateLocationRequest
		...
		if err := updater.UpdateFiltrationLocation(r.Context(), id, req, userID); err != nil {
			...
			if errors.Is(err, storage.ErrUniqueViolation) {
				log.Warn("location name conflict", sl.Err(err))
				render.Status(r, http.StatusConflict)
				render.JSON(w, r, resp.Conflict("Location name already exists"))
				return
			}
			log.Error("failed to update location", sl.Err(err))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, resp.InternalServerError("Failed to update location"))
			return
		}

		log.Info("location updated", slog.Int64("id", id))
