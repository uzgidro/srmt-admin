...
type mockLocationOrgGetterForDelete struct {
	getOrgFunc func(ctx context.Context, id int64) (int64, error)
}

func (m *mockLocationOrgGetterForDelete) GetFiltrationLocationOrgID(ctx context.Context, id int64) (int64, error) {
	if m.getOrgFunc != nil {
...
func TestDelete(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "location not found on org lookup",
			locationID:     "9999",
			orgErr:         storage.ErrNotFound,
			wantStatusCode: http.StatusNotFound,
		},
		{
			name:           "location not found on delete",
			locationID:     "9999",
			orgID:          1,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			deleterMock := &mockLocationDeleter{
				deleteFunc: func(ctx context.Context, id int64) error {
					return tt.mockError
				},
			}
			orgGetterMock := &mockLocationOrgGetterForDelete{
				getOrgFunc: func(ctx context.Context, id int64) (int64, error) {
			...
			r.Delete("/filtration/locations/{id}", Delete(logger, deleterMock, orgGetterMock))
