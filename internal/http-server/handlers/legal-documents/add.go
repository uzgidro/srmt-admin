...
func Add(log *slog.Logger, adder documentAdder, uploader fileupload.FileUploader, saver fileupload.FileMetaSaver, categoryGetter fileupload.CategoryGetter) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			if uploadResult != nil {
				log.Warn("legal document creation failed, compensating uploaded files")
				fileupload.CompensateEntityUpload(r.Context(), log, uploader, saver, uploadResult)
			}
		...
		if len(fileIDs) > 0 {
			if err := adder.LinkLegalDocumentFiles(r.Context(), id, fileIDs); err != nil {
...
// parseMultipartAddRequest parses legal document data from multipart form and handles file uploads
func parseMultipartAddRequest(
	...
	const op = "legal-documents.parseMultipartAddRequest"
