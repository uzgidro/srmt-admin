...
type ConfigDeleter interface {
	DeleteSolarConfig(ctx context.Context, organizationID int64) error
}

func DeleteConfig(log *slog.Logger, repo ConfigDeleter) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		const op = "handlers.solar.DeleteConfig"
		...
		if !callerIsAdmin(r.Context()) {
			...
			log.Warn("non-admin attempted solar config delete",
		...
		if err := repo.DeleteSolarConfig(r.Context(), orgID); err != nil {
			if errors.Is(err, storage.ErrNotFound) {
				render.Status(r, http.StatusNotFound)
				render.JSON(w, r, resp.NotFound("config not found"))
				return
			}
			log.Error("failed to delete solar config", sl.Err(err))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, resp.InternalServerError("failed to delete config"))
			return
		}

		log.Info("solar config deleted", slog.Int64("organization_id", orgID))
